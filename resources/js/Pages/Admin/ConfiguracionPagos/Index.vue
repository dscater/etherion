...
const responseConfiguracionPagos = ref([]);
...
const loadItems = async ({ page, itemsPerPage, sortBy }) => {
    ...
    setTimeOutLoadData = setTimeout(async () => {
        responseConfiguracionPagos.value = await getConfiguracionPagosApi(
            options.value
        );
        listConfiguracionPagos.value = responseConfiguracionPagos.value.data;
        totalItems.value = parseInt(responseConfiguracionPagos.value.total);
        loading.value = false;
    }, 300);
};
const recargaConfiguracionPagos = async () => {
    loading.value = true;
    listConfiguracionPagos.value = [];
    options.value.search = search.value;
    responseConfiguracionPagos.value = await getConfiguracionPagosApi(
        options.value
    );
...
const editarConfiguracionPago = (item) => {
    setConfiguracionPago(item);
    accion_dialog.value = 1;
    open_dialog.value = true;
};
const eliminarConfiguracionPago = (item) => {
    Swal.fire({
        title: "¿Quierés eliminar este registro?",
        html: `<strong>${item.banco} - ${item.nro_cuenta}</strong>`,
        ...
        if (result.isConfirmed) {
            let respuesta = await deleteConfiguracionPago(item.id);
            if (respuesta && respuesta.sw) {
                recargaConfiguracionPagos();
            }
        }
    });
};
...
<template>
    ...
    <v-container>
                                ...
                                <tr v-if="!mobile">
                                    ...
                                    <td class="text-right">
                                        ...
                                        <v-btn
                                            ...
                                            class="pa-1 ma-1"
                                            @click="
                                                eliminarConfiguracionPago(item)
                                            "
                                            icon="mdi-trash-can"
                                        ></v-btn>
                                    </td>
                                ...
                                <tr v-else>
                                    <td>
                                        <ul class="flex-content">
                                            ...
                                            <li
                                                class="flex-item"
                                                data-label="Nombre de Banco:"
                                            >
                                                {{ item.banco }}
                                            </li>
                                            <li
                                                class="flex-item"
                                                data-label="Nro. de cuenta:"
                                            >
                                                {{ item.nro_cuenta }}
                                            </li>
                                            <li
        ...
        <Formulario
            ...
            @envio-formulario="recargaConfiguracionPagos"
